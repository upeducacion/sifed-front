...
import { bibliotecaApi, Recurso } from "@/lib/api/biblioteca";
import { getStorageUrl, cn } from "@/lib/utils";
import { useToast } from "@/hooks/use-toast";
import { handleApiError } from "@/lib/error-handler";
import Loader from "@/components/ui/loader";
import { tipoMeta, COLECCION_LABELS } from "@/lib/repositorio/tipos";
...
export default function BibliotecaAdminPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    ['/api/admin/biblioteca', { search, categoria_id: categoriaId, coleccion, page, per_page: 10 }],
  ...
  const recursos: Recurso[] = data?.data || [];
  const pagination = {
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <select
              ...
              value={coleccion}
              onChange={(e) => { setColeccion(e.target.value); setPage(1); }}
            >
              <option value="">Todas las colecciones</option>
              <option value="investigacion">{COLECCION_LABELS.investigacion}</option>
              <option value="biblioteca">{COLECCION_LABELS.biblioteca}</option>
            </select>
          </div>
          <div className="relative">
            <select
              className="pl-4 pr-8 py-2.5 bg-muted/30 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all text-sm appearance-none cursor-pointer"
              value={categoriaId}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-brand-50/50 border-b border-border">
                ...
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-wider text-brand-900">Tipo / Colección</th>
            ...
            <tbody className="divide-y divide-border">
              {isLoading && recursos.length === 0 ? (
                <tr><td colSpan={6}><Loader text="Cargando biblioteca..." /></td></tr>
              ) : recursos.length === 0 ? (
                <tr>
                  ...
                  <tr key={item.id} className="hover:bg-muted/30 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1.5 items-start">
                        <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest border", tipoMeta(item.tipo).badgeClass)}>
                          {tipoMeta(item.tipo).label}
                        </span>
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                          {COLECCION_LABELS[item.coleccion]}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
