...
export default function EditDocentePage({ params }: EditDocentePageProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    estado: "activo",
    orden: 0,
    enlaces: [] as { red: string, url: string }[],
    config_visibilidad: {
      mostrar_cv: false,
      mostrar_bio: false,
      mostrar_redes: false
  ...
  useEffect(() => {
    ...
    const fetchDocente = async () => {
      try {
        ...
        // Parsear enlaces desde objeto a array de objetos { red, url }
        const enlacesObj = docente.detalles_json?.enlaces_sociales || {};
        const enlacesArray = Object.keys(enlacesObj).length > 0
          ? Object.entries(enlacesObj).map(([red, url]) => ({ red, url: url as string }))
          : [{ red: "Linkedin", url: "" }, { red: "ORCID", url: "" }, { red: "Google Scholar", url: "" }];

        setFormData({
          nombre_completo: docente.nombre_completo || "",
          grados: gradosArray.length > 0 ? gradosArray : [""],
          biografia: docente.biografia || "",
          categoria: docente.categoria || "principal",
          estado: docente.estado || "activo",
          orden: docente.orden || 0,
          enlaces: enlacesArray,
          config_visibilidad: {
            mostrar_cv: docente.config_visibilidad?.mostrar_cv || false,
            mostrar_bio: docente.config_visibilidad?.mostrar_bio || false,
            mostrar_redes: docente.config_visibilidad?.mostrar_redes || false
          }
        });
  ...
  if (initialLoading) {
    return (
      <div className="w-full py-20 flex flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground font-medium">Cargando datos del docente...</p>
      </div>
    );
  }
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-border shadow-sm space-y-6">
            ...
            <div className="space-y-2">
              ...
              >
                <option value="activo">Activo (Visible en portal)</option>
                <option value="inactivo">Inactivo (Oculto en portal)</option>
              </select>
            </div>
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-3 p-4 bg-brand-50/50 border border-dashed border-brand-200 rounded-xl transition-all hover:bg-brand-50">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-brand-900 truncate">
                        {cvFile ? cvFile.name : "Subir nuevo CV (Reemplazará el actual)"}
                      </p>
