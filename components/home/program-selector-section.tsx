        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat) => (
            <Link 
              ...
              className={`group p-8 rounded-lg border-2 transition-all duration-200 hover:shadow-xl hover:-translate-y-1 flex flex-col justify-between min-h-[320px] ${cat.color}`}
            >
              <div>
                <div className={`w-14 h-14 rounded-lg flex items-center justify-center mb-8 transition-transform duration-200 group-hover:scale-[1.03] ${cat.title === 'Doctorados' ? 'bg-white/10' : 'bg-brand-50'}`}>
                  {cat.icon}
                </div>
